...
/* 순번그리드/ 클릭한번 에디팅 / 페이지네이션 S */
/* const sample1 = new tui.Grid({
    el: document.getElementById('sample1'),
    scrollX: false,
    scrollY: false,
	bodyHeight: 200,
	pageOptions: { 
		useClient: true, // paginaton on -- perPage 수만큼 자동 pagination 됨
		perPage: 5  // pagination 나눌 페이지 수
	},
    rowHeaders: [
    ...
    columns : [
		{
			...
			name: 'userId',
			editor : 'text',
			align : 'center',// 컬럼값 가운데 정렬 left, right (default값:left)
		},
		{
			header: '사번',
			name: 'empNo',
		...
		},
		{
			header: '부서',
			name: 'deptCode',
			formatter: 'listItemText', // 이걸 써야 item과 value 매칭
			editor : {
			      type: 'select',
			      options: {
			        listItems: deptList() //ex) item : 보여질 이름 / value : 코드값
			      }
			    },
		},
		{
	  ...
	  header: {
		  height: 100,
		    complexColumns: [ // colunm 헤드 병합
		      {
		        header: '순번 그리드 / 클릭한번 에디팅 / 페이지네이션',
		        name: 'parent2',
		        childNames: ['userId', 'empNo', 'userNm', 'deptCode' ,'clsfCode', 'ofcpsCode', 'jncmpYmd','sttusCode']
		      }
		    ]
		  }, 
	columnOptions: {},
  	data : { api : { // readData 외의 함수는 그리드 문서에서 확인
		   		readData : { url: '/orgChart/selectSampleUserInfo.do', method : 'POST'}
	   		},
			contentType: 'application/json',
			headers: {'X-CSRF-TOKEN' : CRSF_VALUE} // ajax 요청시 꼭 필요
	   } 
  }); */
  /* 순번그리드/ 클릭한번 에디팅 / 페이지네이션 E */
  
  /* 체크박스 그리드/ 클릭한번 에디팅 / 스크롤링 S */
 /* const sample2 = new tui.Grid({
			    ...
			    complexColumns: [ // colunm 헤드 병합
			      {
			        header: '사용기간',
			        name: 'parent1',
			        childNames: ['시작일자1','종료일자1']
			      },
			      {
				        header: '지표설정기간',
				        name: 'parent2',
				        childNames: ['시작일자2','종료일자2']
				  },
				  {
				        header: '실적입력기간',
				        name: 'parent3',
				        childNames: ['시작일자3','종료일자3']
				  },
				  {
				        header: '1차 평가기간',
				        name: 'parent4',
				        childNames: ['시작일자4','종료일자4']
				  },
				  {
				        header: '이의제기기간',
				        name: 'parent5',
				        childNames: ['시작일자5','종료일자5']
				  },
				  {
				        header: '2차 평가기간',
				        name: 'parent6',
				        childNames: ['시작일자6','종료일자6']
				  }
  ...
  //Lee sample2
  /* 체크박스 그리드/ 클릭한번 에디팅 / 스크롤링 E */
  
  /* 순번체크박스 그리드/ 클릭한번 에디팅 / 스크롤링 S */
        ...
        columns : [
		  ...
		  header: {
			  height: 100,
			    complexColumns: [ // colunm 헤드 병합
			      {
			        header: '순번체크박스 그리드 / 클릭한번 에디팅 / 스크롤',
			        name: 'parent2',
			        childNames: ['userId', 'empNo', 'userNm', 'deptCode' ,'clsfCode', 'ofcpsCode', 'jncmpYmd','sttusCode']
			      }
			    ]
...
// 		columnOptions: { resizable: true },
  ...
  /* 순번/체크박스/필터/드래그앤드롭/리사이징/rowSpan/컬럼병합 row 추가/삭제 S */
    const sample4 = new tui.Grid({
        el: document.getElementById('sample4'),
        scrollX: false,
        scrollY: true,
		bodyHeight: 200,
		pageOptions: {
		...
		draggable: true, // drag and drop 기능 
        rowHeaders: [
        	{ type : 'rowNum', header : "순번", width: 50, align : 'center'},
        	{ type : 'checkbox', header : `
                <label for="all-checkbox" class="checkbox">
       ...
       // editingEvent: 'click', - 셀 한번 클릭시 에딧 이벤트 실행 / default 두번 클릭 
        columns : [
			{
				...
				filter: {
			        ...
			        showApplyBtn: true,
			        showClearBtn: true,
			        operator: 'OR' // 복수조건 필터 설정 값 select를 제외한 text, number, date 만 사용 가능
			      },
			      validation: {
						dataType: 'string',
		     		     required: true
			...
			},
			{
				...
				resizable: false // 컬럼별로 resize 기능 활용 가능
			},
		],
		  header: {
			  height: 160,
			    complexColumns: [ // colunm 헤드 병합
			      {
			        header: '복합컬럼1', // 병합할 컬럼명
			        name: 'parent1', // 컬럼 선택자
			        childNames: ['userId', 'empNo', 'userNm']  // 병합할 자식 컬럼          
			      },
			      {
			        header: '순번,체크박스,필터,드래그앤드롭, 리사이징, rowSpan, 컬럼병합, 스크롤',
			        name: 'parent2',
			        childNames: ['userId', 'empNo', 'userNm', 'deptCode' ,'clsfCode', 'ofcpsCode', 'jncmpYmd','sttusCode']
			      }
			    ]
		...
		columnOptions: { 
			resizable: true // 컬럼 resize 전체 사용
	      },
      	data :{ api : { // readData 외의 함수는 그리드 문서에서 확인
			   		readData : { url: '/orgChart/selectSampleUserInfo.do', method : 'POST'}
		   		},
 ...
 /* 순번/체크박스/필터/드래그앤드롭/리사이징/rowSpan/컬럼병합 row 추가/삭제 E */
